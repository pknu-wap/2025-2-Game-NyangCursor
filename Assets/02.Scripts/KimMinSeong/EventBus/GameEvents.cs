...
{
    // 플레이어 관련
    OnPlayerStartOverdrive,
    OnPlayerFinishOverdrive,
    OnPlayerTakeDamage,
...
public static class GameEvents
{
    ...
    private static Dictionary<(GameEventType, Type), Delegate> events = new();

    // 매개변수가 있는 이벤트를 구독할 때 사용
    public static void Subscribe<T>(GameEventType eventType, Action<T> handler)
    {
    ...
    public static void Subscribe(GameEventType eventType, Action handler)
    {
    ...
    public static void Unsubscribe<T>(GameEventType eventType, Action<T> handler)
    {
        var key = (eventType, typeof(T));
    ...
    // ※ 메모리 누수를 방지하려면 꼭 Subscribe 를 호출한 객체에서 Unsubscribe 도 호출해주어야 함
    public static void Unsubscribe(GameEventType eventType, Action handler)
    {
        var key = (eventType, typeof(void));
    ...
    public static void Publish<T>(GameEventType eventType, T data)
    {
        var key = (eventType, typeof(T));

...
public enum FieldObjectEventType
{
    OnBreak,
    // 필요시 추가
}

// 필드 오브젝트와 관련된 이벤트를 관리하는 이벤트 버스 클래스 (ex. 항아리 등등에 사용)
public class FieldObjectEvents
{
    // Delegate 타입은 메서드를 참조할 수 있는 타입임. 즉, 함수 포인터 역할을 수행함
    // (이벤트 타입, 매개변수 타입) 을 키로 사용
    // 따라서 Dictionary 를 사용해 해당되는 키에 콜백 함수들을 매핑함
    private Dictionary<(FieldObjectEventType, Type), Delegate> events = new();

    // 매개변수가 있는 이벤트를 구독할 때 사용
    public void Subscribe<T>(FieldObjectEventType eventType, Action<T> handler)
    {
    ...
    public void Subscribe(FieldObjectEventType eventType, Action handler)
    {
    ...
    public void Unsubscribe<T>(FieldObjectEventType eventType, Action<T> handler)
    {
        var key = (eventType, typeof(T));
    ...
    // ※ 메모리 누수를 방지하려면 꼭 Subscribe 를 호출한 객체에서 Unsubscribe 도 호출해주어야 함
    public void Unsubscribe(FieldObjectEventType eventType, Action handler)
    {
        var key = (eventType, typeof(void));
    ...
    public void Publish<T>(FieldObjectEventType eventType, T data)
    {
        var key = (eventType, typeof(T));
    ...
    public void Publish(FieldObjectEventType eventType)
    {
        var key = (eventType, typeof(void));
        if (events.TryGetValue(key, out var handler))

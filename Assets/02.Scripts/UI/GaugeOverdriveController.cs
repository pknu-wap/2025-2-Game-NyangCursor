...
public class GaugeOverdriveController : MonoBehaviour
{
    ...
    public CursorController mover; // 선택: 이동 스크립트
    ...
    private float baseSpeed = -1f;     // mover 속도 원본 저장
    ...
    void Start()
    {
        if (mover != null) baseSpeed = mover.speed;
        ClampAll();
    ...
    void Update()
    {
        ...
        else
        {
            ...
            {
                odSinceRelease += dt;
                if (odSinceRelease >= odDecayDelay)
    ...
    void ApplyTierSpeed(int tier)
    {
        ...
        if (baseSpeed < 0f) baseSpeed = mover.speed; // 안전장치
        ...
        mover.speed = baseSpeed * mul; // 구간 동안 '유지'되는 계단식 속도
    }
    ...
    void TriggerBoostObject()
    {
        ...
        boostRoutine = StartCoroutine(BoostObjectRoutine());
    }
